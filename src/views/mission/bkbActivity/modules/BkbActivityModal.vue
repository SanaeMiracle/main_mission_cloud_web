    ...
    <a-spin :spinning='confirmLoading'>
      <a-form-model ref='form' :model='model' :rules='validatorRules'>
        ...
        <a-form-model-item label='计划开始日期' :labelCol='labelCol' :wrapperCol='wrapperCol' prop='startDate'>
          <j-date placeholder='请选择计划开始日期' :disabled-date='disabledEndDate' v-model='model.startDate'
                  :show-time='true'
        ...
        <a-form-model-item label='活动时长' :labelCol='labelCol' :wrapperCol='wrapperCol' prop='activityTime'>
          <a-input
            ...
            v-model='model.activityTime' placeholder='请输入活动时长'></a-input>
        </a-form-model-item>
        ...
        <a-form-model-item
          v-if="model.portId !== 'C' && model.portId !== 'D' && (model.status==10 || model.status==30 || !model.status)"
          label='演讲员' :labelCol='labelCol'
  ...
  methods: {
    ...
    disabledRangeTime(v, type) {
      ...
      return {
        ...
        disabledSeconds: () => {
          //时间相等限制秒
          if (parseInt(date_se.getMinutes()) === parseInt(v.format('mm')) && parseInt(date_se.getHours()) === parseInt(v.format('HH'))) {
    ...
    edit(record) {
      ...
      if (record.portId === 'B') {
        getAction('mission/bkbActivity/theExonSubobject', { id: record.id }).then((res) => {

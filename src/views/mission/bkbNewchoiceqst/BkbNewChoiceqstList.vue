...
<!--          </a-col>-->
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
            <a-form-item label="关联培训标题">
              <j-dict-select-tag placeholder="关联培训标题" v-model="queryParam.knowledgeId" dictCode="bkb_training_program,title,id"/>
            </a-form-item>
          </a-col>
          <a-col :xl="6" :lg="7" :md="8" :sm="24">
...
<!--&lt;!&ndash;              <a-form-item label="地区">&ndash;&gt;-->
<!--&lt;!&ndash;                <j-search-select-tag :dictOptions='searchOptions'  v-model="queryParam.region" placeholder="请选择省市区"/>&ndash;&gt;-->
...
<script>
  ...
  import BkpChoiceqstView from './modules/BkbNewChoiceqstView'
  import Area from '@comp/_util/Area'
  import '@assets/less/TableExpand.less'
  import { getAction } from '@api/manage'
  import Vue from 'vue'
  import { USER_INFO } from '@/store/mutation-types'
  import BkpChoiceqstModal from '@views/mission/bkbNewchoiceqst/modules/BkpNewChoiceqstModal'

  export default {
    name: "BkpNewChoiceqstList",
    ...
    data () {
      return {
        ...
        url: {
          list: "/mission/bkbNewChoiceqst/list",
          exportXlsUrl: "/mission/bkbNewChoiceqst/exportXls",

          delete: "/mission/bkbNewChoiceqst/delete",
          deleteBatch: "/mission/bkbNewChoiceqst/deleteBatch",
          importExcelUrl: "mission/bkbNewChoiceqst/importExcel",
          publish: '/mission/bkbNewChoiceqst/publish',
          freeze: '/mission/bkbNewChoiceqst/freeze',
          abandon: '/mission/bkbNewChoiceqst/abandon'
        },
        superFieldList:[],
      }
    },
    ...
    methods: {
      ...
      updateKnowledge() {
        ...
        getAction("/mission/bkbNewChoiceqst/updatedata").then(res => {});
      ...
      getSuperFieldList(){
        let fieldList=[];
        fieldList.push({type:'string',value:'choiceqstTitle',text:'培训题名称',dictCode:''})
        fieldList.push({type:'int',value:'knowledgeId',text:'关联培训标题',dictCode:"bkb_training_program,title,id"})
        // fieldList.push({type:'pca',value:'region',text:'地区'})
        fieldList.push({type:'int',value:'choiceType',text:'题类型',dictCode:'choice_question_type'})
        fieldList.push({type:'string',value:'knowledgeTitle',text:'关联知识库标题',dictCode:''})
        fieldList.push({type:'int',value:'status',text:'题库状态',dictCode:'publish_type_status'})

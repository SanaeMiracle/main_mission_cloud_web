    ...
    <j-form-container :disabled='formDisabled'>
      ...
      <a-form-model ref='form' :model='model' :rules='validatorRules' slot='detail'>
        <a-row>
          ...
          <a-col :span='24' @click='initDictConfig'>
            <a-form-model-item label='关联培训标题' :labelCol='labelCol' :wrapperCol='wrapperCol' prop='knowledgeId'>
<!--              <j-search-select-tag :dictOptions='searchOptions' type='select' v-model='model.knowledgeId'-->
<!--                                 placeholder='请选择关联培训标题' @change='knowTypeChange()' />-->
              <j-dict-select-tag type="list" v-model="model.knowledgeId" :dictCode="`bkb_training_program WHERE status=0,title,id,`" placeholder="请选择关联培训标题" />
            </a-form-model-item>
          </a-col>
          <!--          <a-col :span="24">-->
          ...
          <a-col :span='24'>
            <a-form-model-item label='题型' :labelCol='labelCol' :wrapperCol='wrapperCol' prop='choiceType'>
              <j-dict-select-tag type='radio' v-model='model.choiceType' dictCode='choice_question_type'
...
export default {
  ...
  data() {
    return {
      ...
      validatorRules: {
        ...
        knowledgeId: [
          { required: true, message: '请输入关联培训标题!' }
        ],
        region: [
      ...
      refKeys: ['bkbNewChoiceqstAnswer'],
      tableKeys: ['bkbNewChoiceqstAnswer'],
      activeKey: 'bkbNewChoiceqstAnswer',
      // 选择题答案
      ...
      url: {
        add: '/mission/bkbNewChoiceqst/add',
        edit: '/mission/bkbNewChoiceqst/edit',
        queryById: '/mission/bkbNewChoiceqst/queryById',
        bkbChoiceqstAnswer: {
          list: '/mission/bkbNewChoiceqst/queryBkpChoiceqstAnswerByMainId'
        }
      }
    }
  },
  ...
  methods: {
    knowTypeChange() {
      getAction('/mission/bkbTrainingProgram/queryById').then((res) => {
        this.searchOptions = []
        if (res.success) {

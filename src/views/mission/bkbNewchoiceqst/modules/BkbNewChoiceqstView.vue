    ...
    <template>
      ...
      <div v-if="choiceData">
        <a-descriptions title="题库详情" :column="2">
          <a-descriptions-item label="培训题名称">
            {{ choiceData.choiceqstTitle }}
          </a-descriptions-item>

          <a-descriptions-item label="关联培训标题">
...
export default {
  ...
  methods: {
    initDictConfig(){
      getAction("/mission/bkbNewChoiceqst/queryById", { id: this.choiceqstData.id }).then(res => {
        if (res.success) {
          if(!this.regionData){
            this.regionData = new Area(this.$Jpcaa).getText(res.result.region);
          }
          getAction('/mission/bkbNewChoiceqst/queryBkbChoiceqstAnswerByMainId', { id: this.choiceqstData.id }).then(res => {
            if (res.success) {

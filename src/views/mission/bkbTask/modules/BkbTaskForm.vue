        ...
        <a-row>
          ...
          <a-col :span='24'>
            <a-form-model-item label='任务户数' :labelCol='labelCol' :wrapperCol='wrapperCol' prop='tenement'>
              <a-input-number :min='0' :max='999' style='width: 100%' v-model='model.tenement'
                              placeholder='请输入任务户数'></a-input-number>
            </a-form-model-item>
          </a-col>
          <a-col :span='24'>
...
export default {
  ...
  methods: {
    validateMobile1(rule, value, callback) {
      console.error(value, this.model.selectFloorUnit.length)
      if (value <= this.model.selectFloorUnit.length) {
        callback()
      } else {
        callback('任务数量超出楼栋单元数量!')
      }
    },
    floorunit(value) {
